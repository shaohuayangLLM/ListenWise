...
### ✅ M13 — 热词词表 + AI 术语订正（2026-06-10）

> 背景：0610 智能路由会议实测 vs 讯飞听见，领域实体（千问/硅基流动/星火）大面积误识别，差距归因于热词缺失；订正是引擎短板的后处理补偿。

- **热词词表（全局单行 `glossary` 表）**：设置页「热词词表」卡片，每行一词、上限 500（百炼限额）。`GET/PUT /api/settings/glossary`，保存时去空去重标脏。
